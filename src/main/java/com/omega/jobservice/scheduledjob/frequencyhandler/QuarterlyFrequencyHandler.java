...
public class QuarterlyFrequencyHandler implements FrequencyHandler {
    @Override
    public long nextExecutionTime(long initialExecutionTimeInMillis, long jobTimeInMillis) {
        ...
        initialCalendar.set(Calendar.MONTH, initialCalendar.get(Calendar.MONTH) + 3);
        ...
        while (initialCalendar.getTimeInMillis() <= TimeUtil.currentTimeInMillis()) {
            initialCalendar.add(Calendar.MONTH, 3);
        }
